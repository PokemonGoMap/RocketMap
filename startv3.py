...
try:
    for place in places:
        ...
        execute = "python runserver.py -ns -a %s -u %s -p %s -ss spawns_%d.json -l \"%s,%s\" -st %s -sd %s --dump-spawnpoints" % \
                    (account["auth"], account["login"], account["pw"], x, place["lat"], place["lng"], place["steps"], place["delay"])
