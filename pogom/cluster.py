...
class SpawnCluster(object):
    ...
    def append(self, spawnpoint):
        ...
        self.centroid = clsmath.intermediate_point(spawnpoint.position,
                                                   self.centroid, f)
    ...
    def simulate_centroid(self, spawnpoint):
        f = old_div(len(self._spawnpoints), (len(self._spawnpoints) + 1.0))
        new_centroid = clsmath.intermediate_point(spawnpoint.position,
                                                  self.centroid, f)
...
def check_cluster(spawnpoint, cluster, radius, time_threshold):
    ...
    # we'd be removing x
    if any(clsmath.distance(x.position, new_centroid) > radius for x in
            cluster):
        return False
